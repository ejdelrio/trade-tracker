'use strict';

var app = app||{};
//search 'from:@RSherman_25 @Revis24'

(function(module) {

  var tweets = JSON.stringify([
    {
      "text": "@Revis24 one season u will get 8 picks.... But it won't happen anytime soon... I did it in my 2nd season... So u have something to chase",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "Revis24",
          }
        ]
      },
      "retweet_count": 1799,
      "favourites_count": 703,
      "created_at": "Sat Feb 20 14:07:00 -7000 2013",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@Revis24 don't need ya name... Ur name will be irrelevant once u step back on the field bruh. Get ya picks up!",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "Revis24",
          }
        ]
      },
      "retweet_count": 1659,
      "favourites_count": 549,
      "created_at": "Sat Feb 20 14:06:00 -7000 2013",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "Remember that time the number of twitter followers mattered more than the production on the field? Me either @Revis24",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "Revis24",
          }
        ]
      },
      "retweet_count": 1925,
      "favourites_count": 1128,
      "created_at": "Sat Feb 20 15:38:00 -7000 2013",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@Revis24 thank big dog! Much appreciated.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "Revis24",
          }
        ]
      },
      "retweet_count": 387,
      "favourites_count": 655,
      "created_at": "Sat Feb 03 10:57:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@dsalisbury62 @incarceratedbob @RealPeterson21 @Revis24 yes be is",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "Revis24",
          },
          {
            "screen_name": "dsalisbury62",
          },
          {
            "screen_name": "incarceratedbob",
          },
          {
            "screen_name": "RealPeterson21",
          }
        ]
      },
      "retweet_count": 0,
      "favourites_count": 0,
      "created_at": "Sat Feb 03 10:57:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@cocosunday @Revis24 Thank you",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "Revis24",
          },
          {
            "screen_name": "cocosunday",
          }
        ]
      },
      "retweet_count": 1,
      "favourites_count": 6,
      "created_at": "Sat Sep 29 17:05:00 -7000 2013",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@matt_Condy27 @Revis24 check the stats then talk",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "Revis24",
          },
          {
            "screen_name": "matt_Condy27",
          }
        ]
      },
      "retweet_count": 11,
      "favourites_count": 12,
      "created_at": "Sat Feb 20 16:19:00 -7000 2013",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@its21blackjack @Revis24 ;) they don't throw to his",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "Revis24",
          },
          {
            "screen_name": "its21blackjack",
          }
        ]
      },
      "retweet_count": 5,
      "favourites_count": 8,
      "created_at": "Sat Feb 20 17:05:00 -7000 2013",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@rpchudgar @Revis24 he didn't play this year so when he does then he will be in it",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "Revis24",
          },
          {
            "screen_name": "rpchudgar",
          }
        ]
      },
      "retweet_count": 8,
      "favourites_count": 8,
      "created_at": "Sat Feb 20 09:57:00 -7000 2013",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@DaneCWatson @Revis24 touche",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "Revis24",
          },
          {
            "screen_name": "DaneCWatson",
          }
        ]
      },
      "retweet_count": 0,
      "favourites_count": 0,
      "created_at": "Sat Jan 24 20:06:00 -7000 2013",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@Revis24 praying for a fast and full recovery.... Keep ya head up",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "Revis24",
          }
        ]
      },
      "retweet_count": 2,
      "favourites_count": 3,
      "created_at": "Sat Sep 24 09:11:00 -7000 2012",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "Hey everyone @rsherman_25 wants to be a lockdown corner. He wants to be in the talks of a lockdown corner. So follow him so he can become 1!",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 1324,
      "favourites_count": 433,
      "created_at": "Sat Feb 20 14:32:00 -7000 2013",
      "user": {
        "follower_count": 463110,
        "screen_name": "Revis24",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "Look I just helped you get 6,000 followers homie!!! @rsherman_25",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 1045,
      "favourites_count": 620,
      "created_at": "Sat Feb 20 14:50:00 -7000 2013",
      "user": {
        "follower_count": 463110,
        "screen_name": "Revis24",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "That's like you saying your better then Deion Sanders & since he's not playing no more he's irrelevant for what he's done. @rsherman_25",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 709,
      "favourites_count": 271,
      "created_at": "Sat Feb 20 14:29:00 -7000 2013",
      "user": {
        "follower_count": 463110,
        "screen_name": "Revis24",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "I never seen a man before run his mouth so much like girl. This dude just steady putting my name in his mouth to get notoriety @rsherman_25",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 6697,
      "favourites_count": 3216,
      "created_at": "Sat Feb 20 13:38:00 -7000 2013",
      "user": {
        "follower_count": 463110,
        "screen_name": "Revis24",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@rsherman_25 I ain't your competition bro, @roddywhiteTV is!!!",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "RoddywhiteTV",
          }
        ]
      },
      "retweet_count": 1218,
      "favourites_count": 461,
      "created_at": "Sat Apr 03 13:42:00 -7000 2013",
      "user": {
        "follower_count": 463110,
        "screen_name": "Revis24",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@rsherman_25 child please, lol!!! Your still taking baby steps son. Don't get to a head of yourself.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 785,
      "favourites_count": 297,
      "created_at": "Sat Apr 03 13:42:00 -7000 2013",
      "user": {
        "follower_count": 463110,
        "screen_name": "Revis24",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@LG_Blount I just totally lock down one! And pick off passes that come over there.... Rarely ever giving up completions....no need to follow",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "LG_Blount",
          }
        ]
      },
      "retweet_count": 303,
      "favourites_count": 195,
      "created_at": "Sat Mar 12 18:54:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@LG_Blount I've seen it .... And done it ....",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "LG_Blount",
          }
        ]
      },
      "retweet_count": 316,
      "favourites_count": 339,
      "created_at": "Sat Mar 12 16:54:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@LG_Blount for what? So I can kill the integrity of the defense.... We actually have another good corner who I trust to do his job! We win!",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "LG_Blount",
          }
        ]
      },
      "retweet_count": 207,
      "favourites_count": 219,
      "created_at": "Sat Mar 12 19:13:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@LG_Blount it's all good.... Hard to argue with friendship",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "LG_Blount",
          }
        ]
      },
      "retweet_count": 203,
      "favourites_count": 194,
      "created_at": "Sat Mar 12 19:13:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@JakeParmley_26 @LG_Blount hahaha that's why I am an all pro and SB champ",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "LG_Blount",
          },
          {
            "screen_name": "JakeParmley",
          }
        ]
      },
      "retweet_count": 122,
      "favourites_count": 182,
      "created_at": "Sat Mar 12 19:09:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@LG_Blount hahaha I'm just messing with u man.... Bored",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "LG_Blount",
          },
          {
            "screen_name": "JakeParmley",
          }
        ]
      },
      "retweet_count": 19,
      "favourites_count": 53,
      "created_at": "Sat Mar 11 19:44:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@AustinCampos12 @LG_Blount just opinions no numbers as always",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "LG_Blount",
          },
          {
            "screen_name": "AustinCampos12",
          }
        ]
      },
      "retweet_count": 30,
      "favourites_count": 47,
      "created_at": "Sat Mar 11 19:01:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@AustinCampos12 @LG_Blount hahahaha",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "LG_Blount",
          },
          {
            "screen_name": "AustinCampos12",
          }
        ]
      },
      "retweet_count": 9,
      "favourites_count": 32,
      "created_at": "Sat Mar 11 18:59:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 that one you lock down isn't always the best receiver on the team though... Follow the best receiver every game and do it...",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 437,
      "favourites_count": 238,
      "created_at": "Sat Mar 12 19:11:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "LG_Blount",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 that one you lock down isn't always the best receiver on the team though... Follow the best receiver every game and do it...",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 437,
      "favourites_count": 238,
      "created_at": "Sat Mar 12 19:11:00 -7000 2014",
      "user": {
        "follower_count": 191592,
        "screen_name": "LG_Blount",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 I've seen you do it & he has to. But he do it on both sides. He follow the best receiver on the field every game and strap him.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 262,
      "favourites_count": 131,
      "created_at": "Sat Mar 12 18:52:00 -7000 2014",
      "user": {
        "follower_count": 191592,
        "screen_name": "LG_Blount",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 in my opinion he the best bruh sorry... Can't change my mind...",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 305,
      "favourites_count": 268,
      "created_at": "Sat Mar 12 21:44:00 -7000 2014",
      "user": {
        "follower_count": 191592,
        "screen_name": "LG_Blount",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 He's the best bruh. I've seen what he does to WR/TE in person and I'm sure you have too. Friendship aside he's the best...",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 276,
      "favourites_count": 199,
      "created_at": "Sat Mar 12 13:19:00 -7000 2014",
      "user": {
        "follower_count": 191592,
        "screen_name": "LG_Blount",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 who's the other good corner? We aren't even talking about whoever that is. It doesn't matter. We talking about the best Talib!!",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 146,
      "favourites_count": 153,
      "created_at": "Sat Mar 12 19:30:00 -7000 2014",
      "user": {
        "follower_count": 191592,
        "screen_name": "LG_Blount",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "“@Chezsports: @LG_Blount @KurtMcDonough @RSherman_25 Smith caught four passes” thank you!",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "Chezsports",
          },
          {
            "screen_name": "KurtMcDonough",
          },
          {
            "screen_name": "LG_Blount",
          }
        ]
      },
      "retweet_count": 146,
      "favourites_count": 153,
      "created_at": "Sat Mar 12 19:30:00 -7000 2014",
      "user": {
        "follower_count": 191592,
        "screen_name": "LG_Blount",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@LaMichaelJames Aqib the coldest bruh and @RSherman_25  knows that",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "LaMichaelJames",
          }
        ]
      },
      "retweet_count": 75,
      "favourites_count": 49,
      "created_at": "Sat Mar 12 19:40:00 -7000 2014",
      "user": {
        "follower_count": 191592,
        "screen_name": "LG_Blount",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "“@KurtMcDonough: @RSherman_25 @LG_Blount yo let's not forget Talib during that panthers game, Steve smith did work.” How many yards?",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "KurtMcDonough",
          }
        ]
      },
      "retweet_count": 14,
      "favourites_count": 41,
      "created_at": "Sat Mar 12 19:40:00 -7000 2014",
      "user": {
        "follower_count": 191592,
        "screen_name": "LG_Blount",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@LaMichaelJames Aqib the coldest bruh and @RSherman_25  knows that",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "LaMichaelJames",
          }
        ]
      },
      "retweet_count": 75,
      "favourites_count": 49,
      "created_at": "Sat Mar 12 19:40:00 -7000 2014",
      "user": {
        "follower_count": 191592,
        "screen_name": "LG_Blount",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 @AustinCampos12 Julio Jones & Jimmy Graham.... #STRAPPED",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "AustinCampos12",
          }
        ]
      },
      "retweet_count": 53,
      "favourites_count": 81,
      "created_at": "Sat Mar 12 19:18:00 -7000 2014",
      "user": {
        "follower_count": 191592,
        "screen_name": "LG_Blount",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 @JakeParmley_26 was Aqib not an all pro/pro bowler this year? I think he was... That's why he got paid like the best... He is..",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "JakeParmley_26",
          }
        ]
      },
      "retweet_count": 56,
      "favourites_count": 44,
      "created_at": "Sat Mar 12 19:13:00 -7000 2014",
      "user": {
        "follower_count": 191592,
        "screen_name": "LG_Blount",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "“@nathanthorne69: @LG_Blount @Tmoney92080 what you smoking blount ahah die hard pats fan Talib got nothing on @RSherman_25” opinions vary 😒😒",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "nathanthorne69",
          },
          {
            "screen_name": "LG_Blount",
          },
          {
            "screen_name": "Tmoney92080",
          }
        ]
      },
      "retweet_count": 9,
      "favourites_count": 41,
      "created_at": "Sat Mar 12 21:11:00 -7000 2014",
      "user": {
        "follower_count": 191592,
        "screen_name": "LG_Blount",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@DeAngeloHall23 let me know when u ever have an 8 pick season.... Please....I'll be awaiting that call",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "DeAngeloHall23",
          }
        ]
      },
      "retweet_count": 2782,
      "favourites_count": 1500,
      "created_at": "Sat Mar 13 18:24:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@DeAngeloHall23 been tough on u huh lol",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "DeAngeloHall23",
          }
        ]
      },
      "retweet_count": 879,
      "favourites_count": 551,
      "created_at": "Sat Mar 13 18:13:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@DeAngeloHall23 is that all u had that season?",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "DeAngeloHall23",
          }
        ]
      },
      "retweet_count": 1938,
      "favourites_count": 1801,
      "created_at": "Sat Mar 13 18:28:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@DeAngeloHall23 I checked ya film! It was rough.... Have some pride bout ya play",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "DeAngeloHall23",
          }
        ]
      },
      "retweet_count": 1457,
      "favourites_count": 755,
      "created_at": "Sat Mar 13 18:16:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@DeAngeloHall23 I just see a lot of Ls",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "DeAngeloHall23",
          }
        ]
      },
      "retweet_count": 1575,
      "favourites_count": 1169,
      "created_at": "Sat Mar 13 18:17:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@DeAngeloHall23 we all do.  Just make sure it's a rare occurrence ....",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "DeAngeloHall23",
          }
        ]
      },
      "retweet_count": 1575,
      "favourites_count": 1169,
      "created_at": "Sat Mar 13 16:44:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@Realrclark25 @DeAngeloHall23 I'm the best at what every coach wants u to do.... Catch a lot of ints and not give up Tds...try it",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "DeAngeloHall23",
          },
          {
            "screen_name": "Realrclark25",
          }
        ]
      },
      "retweet_count": 823,
      "favourites_count": 696,
      "created_at": "Sat Mar 13 16:44:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@DeAngeloHall23 preciate that",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "DeAngeloHall23",
          }
        ]
      },
      "retweet_count": 36,
      "favourites_count": 78,
      "created_at": "Sat May 7 15:54:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RainRiotHawk @ESPNNFL @DeAngeloHall23 hahaha crazy right... But it's not about facts...just enjoy talking nonsense",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "DeAngeloHall23",
          },
          {
            "screen_name": "RainRiotHawk",
          },
          {
            "screen_name": "ESPNNFL",
          }
        ]
      },
      "retweet_count": 2,
      "favourites_count": 3,
      "created_at": "Sat Mar 15 12:00:00 -7000 2014",
      "user": {
        "follower_count": 1912435,
        "screen_name": "RSherman_25",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 let me know when you get 4INTs in one game.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 3409,
      "favourites_count": 1941,
      "created_at": "Sat Mar 13 18:27:00 -7000 2014",
      "user": {
        "follower_count": 103044,
        "screen_name": "DeAngeloHall23",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 no beef at all young fella. I'm a dog you bark at me I'm barking back PERIOD....",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 1593,
      "favourites_count": 1041,
      "created_at": "Sat Mar 13 17:50:00 -7000 2014",
      "user": {
        "follower_count": 103044,
        "screen_name": "DeAngeloHall23",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 just make sure you stay in Seattle cause without that pass rush you are who we thought you were. 5th rd PK no speed good hands",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 5402,
      "favourites_count": 2854,
      "created_at": "Sat Mar 13 18:20:00 -7000 2014",
      "user": {
        "follower_count": 103044,
        "screen_name": "DeAngeloHall23",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 not at all young fella. Check my stats. Then after that go check my BANK account.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 4896,
      "favourites_count": 2779,
      "created_at": "Sat Mar 13 18:15:00 -7000 2014",
      "user": {
        "follower_count": 103044,
        "screen_name": "DeAngeloHall23",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "Congrats @RSherman_25 I'm all about guys getting paid. You said you the best, they paid you like you the best. That's all that matters. $$$",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 294,
      "favourites_count": 269,
      "created_at": "Sat May 7 11:13:00 -7000 2014",
      "user": {
        "follower_count": 103044,
        "screen_name": "DeAngeloHall23",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 man you better start praying now that you can do what I do 10 years in the game.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          }
        ]
      },
      "retweet_count": 1006,
      "favourites_count": 537,
      "created_at": "Sat Mar 13 18:25:00 -7000 2014",
      "user": {
        "follower_count": 103044,
        "screen_name": "DeAngeloHall23",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@ochocinco @RSherman_25 all I'm saying to the kid is I want to see him against top guys all day chad like we use to do.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "ochocinco",
          }
        ]
      },
      "retweet_count": 730,
      "favourites_count": 479,
      "created_at": "Sat Mar 13 18:30:00 -7000 2014",
      "user": {
        "follower_count": 103044,
        "screen_name": "DeAngeloHall23",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@justin_beachy @RSherman_25 no need to we all saw it.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "justin_beachy",
          }
        ]
      },
      "retweet_count": 45,
      "favourites_count": 44,
      "created_at": "Sat Sep 14 19:34:00 -7000 2014",
      "user": {
        "follower_count": 103044,
        "screen_name": "DeAngeloHall23",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@ochocinco @RSherman_25 you win some you lose some. But you get that respect for doing it. And you get that money. I want him to get paid",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "ochocinco",
          }
        ]
      },
      "retweet_count": 382,
      "favourites_count": 212,
      "created_at": "Sat Mar 13 18:31:00 -7000 2014",
      "user": {
        "follower_count": 103044,
        "screen_name": "DeAngeloHall23",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 yeah you right your team is great man. Can't argue that. @Kam_Chancellor @Earl_Thomas they the Best in the biz.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "Kam_Chancellor",
          },
          {
            "screen_name": "Earl_Thomas",
          }
        ]
      },
      "retweet_count": 484,
      "favourites_count": 308,
      "created_at": "Sat Mar 13 18:16:00 -7000 2014",
      "user": {
        "follower_count": 103044,
        "screen_name": "DeAngeloHall23",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RSherman_25 The only CB that doesn't get beat, is the one that's sitting the bench. Ask @DeionSanders or @darrellgreen28",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "DeionSanders",
          },
          {
            "screen_name": "darrellgreen28",
          }
        ]
      },
      "retweet_count": 708,
      "favourites_count": 371,
      "created_at": "Sat Mar 13 16:12:00 -7000 2014",
      "user": {
        "follower_count": 103044,
        "screen_name": "DeAngeloHall23",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "I can't lie @RSherman_25 the world wants to see you lock up with guys like @Bigplaycj @DezBryant @ajgreen_18 all game long. I do too!!!!!!",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "Bigplaycj",
          },
          {
            "screen_name": "DezBryant",
          },
          {
            "screen_name": "ajgreen_18",
          }
        ]
      },
      "retweet_count": 946,
      "favourites_count": 477,
      "created_at": "Sat Mar 13 16:09:00 -7000 2014",
      "user": {
        "follower_count": 103044,
        "screen_name": "DeAngeloHall23",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@AmandaCoppinger lol thanks for the love and support. @RSherman_25 can get that ball. He's a #ballhawk I love it",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RSherman_25",
          },
          {
            "screen_name": "AmandaCoppinger",
          }
        ]
      },
      "retweet_count": 0,
      "favourites_count": 3,
      "created_at": "Sat Feb 4 13:23:00 -7000 2014",
      "user": {
        "follower_count": 103044,
        "screen_name": "DeAngeloHall23",
        "time_zone": "Pacific",
      },
    },

    //Start of TMobile/Sprint tweets//
    {
      "text": "Pro-tip for @marceloclaure, tell people how to pronounce your name now... #leegair!?",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          }
        ]
      },
      "retweet_count": 15,
      "favourites_count": 40,
      "created_at": "Sat Aug 6 10:56:00 -7000 2014",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "1/ @TMobile has surpassed @Sprint – we’re now #1 in prepaid wireless! Sorry @marceloclaure, you're already behind! http://t-mo.co/X2EwWg",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "Sprint",
          }
        ]
      },
      "retweet_count": 87,
      "favourites_count": 118,
      "created_at": "Sat Aug 6 16:38:00 -7000 2014",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@XPCLLC @TMobile @sprint @marceloclaure and you follow me why?",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "TMobile",
          },
          {
            "screen_name": "XPCLLC",
          }
        ]
      },
      "retweet_count": 2,
      "favourites_count": 10,
      "created_at": "Sat Aug 15 17:57:00 -7000 2014",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@bigehays @michello67 @jessdee71 @marceloclaure why don't you come over? You have fight. I like that. Wanna join #Magenta?",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "bigehays",
          },
          {
            "screen_name": "michelleo67",
          },
          {
            "screen_name": "jessdee71",
          }
        ]
      },
      "retweet_count": 1,
      "favourites_count": 4,
      "created_at": "Sat Aug 17 18:05:00 -7000 2014",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@marceloclaure @sprint Nexttime I rally team,i'll READ notecards,Confidence in leader #NOT ,StiffScared #Boring #yawn",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "sprint",
          },
        ]
      },
      "retweet_count": 7,
      "favourites_count": 10,
      "created_at": "Sat Aug 19 18:03:00 -7000 2014",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@krisspragg @marceloclaure @sprint was a great speech from the heart and worked very well",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "sprint",
          },
          {
            "screen_name": "krisspragg",
          }
        ]
      },
      "retweet_count": 2,
      "favourites_count": 1,
      "created_at": "Sat Aug 19 18:20:00 -7000 2014",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@mikefranze4 @marceloclaure @sprint zero comment, no retweets, no interest #frorible",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "sprint",
          },
          {
            "screen_name": "mikefranze4",
          }
        ]
      },
      "retweet_count": 1,
      "favourites_count": 4,
      "created_at": "Sat Sep 29 21:41:00 -7000 2014",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RogerWCheng @marceloclaure good try Roger... It's about customers not CEOs.. And besides the text might not be received on @sprint network",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "RogerWCheng",
          },
          {
            "screen_name": "sprint",
          }
        ]
      },
      "retweet_count": 5,
      "favourites_count": 14,
      "created_at": "Sat Jan 23 10:29:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@marceloclaure @sprint Oh look! You learned how to use the “respond” button. I thought you only knew how to use the retweet button.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "sprint",
          }
        ]
      },
      "retweet_count": 31,
      "favourites_count": 78,
      "created_at": "Sat Feb 1 19:23:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@marceloclaure @sprint Yes – maybe you should keep this in mind when you RT people talking about @KimKardashian.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "sprint",
          },
          {
            "screen_name": "KimKardashian",
          }
        ]
      },
      "retweet_count": 13,
      "favourites_count": 40,
      "created_at": "Sat Feb 1 19:40:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RogerWCheng @marceloclaure you mean me and his staff...",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "RogerWCheng",
          }
        ]
      },
      "retweet_count": 10,
      "favourites_count": 30,
      "created_at": "Sat Feb 1 19:54:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": ".@RogerWCheng @marceloclaure Do any of you really think @marceloclaure claure touches his twitter ? Do the 99% retweets really fool you?",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "RogerWCheng",
          }
        ]
      },
      "retweet_count": 22,
      "favourites_count": 54,
      "created_at": "Sat Feb 1 20:19:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@marceloclaure @RogerWCheng wow that 3 day delayed reply makes twitter so useful...what's next happy new year?",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "RogerWCheng",
          }
        ]
      },
      "retweet_count": 22,
      "favourites_count": 54,
      "created_at": "Sat Feb 4 20:48:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": ".@RogerWCheng @brettschulte @marceloclaure thanks for being so fair Ro",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "RogerWCheng",
          },
          {
            "screen_name": "brettschulte",
          }
        ]
      },
      "retweet_count": 3,
      "favourites_count": 13,
      "created_at": "Sat Feb 5 16:55:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": ".@BagirovEmmanuel @marceloclaure doesn't seem like a rude awakening.Our 4G and your 3G did as expected. I have an announcement for you.Wait!",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "BagirovEmmanuel",
          }
        ]
      },
      "retweet_count": 14,
      "favourites_count": 37,
      "created_at": "Sat Feb 9 23:33:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "Did @masason say there was going to be a “rude awakening” and @marceloclaure assumed it was us? #swingandamiss http://on.wsj.com/1A9nzeD ",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "masason",
          }
        ]
      },
      "retweet_count": 10,
      "favourites_count": 29,
      "created_at": "Sat Feb 10 10:12:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@marceloclaure I react fast and constant. It's what i do.yes ,fix your network. We all need to. Can you congratulate me for being number 3?",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          }
        ]
      },
      "retweet_count": 17,
      "favourites_count": 21,
      "created_at": "Sat Feb 10 21:05:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": ".@marceloclaure Your unlimited plan = $60. Your unlimited plan for iPhone = $50. Your unlimited plan for free #GS6 = $80. That's simple?!",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          }
        ]
      },
      "retweet_count": 36,
      "favourites_count": 106,
      "created_at": "Sat Mar 26 20:47:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": ".@marceloclaure @EssentialDacome and really really slow if you can get coverage :)",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "EssentialDacome",
          }
        ]
      },
      "retweet_count": 9,
      "favourites_count": 49,
      "created_at": "Sat Mar 26 20:52:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@marceloclaure #trickery you made my point thank you.. You didn't mention lease vs buy and one ends with no ownership #fineprintlies #bs",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          }
        ]
      },
      "retweet_count": 10,
      "favourites_count": 38,
      "created_at": "Sat Mar 26 21:18:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": ".@marceloclaure and I repeat it is omg horribly slow and spotty and unusable network #leaselies #SloworNoNetwork #sorrynotsorry",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          }
        ]
      },
      "retweet_count": 21,
      "favourites_count": 76,
      "created_at": "Sat Mar 26 21:20:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "Half-assed commercial, half-assed data speeds. #typical @sprint. #SprintLikeHell",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "marceloclaure",
          },
          {
            "screen_name": "sprint",
          }
        ]
      },
      "retweet_count": 242,
      "favourites_count": 386,
      "created_at": "Sat Feb 1 18:17:00 -7000 2015",
      "user": {
        "follower_count": 4194117,
        "screen_name": "JohnLegere",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@JohnLegere @sprint Never sacrifice your class to get even with someone who has none.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "JohnLegere",
          },
          {
            "screen_name": "sprint",
          }
        ]
      },
      "retweet_count": 169,
      "favourites_count": 228,
      "created_at": "Sat Feb 1 18:32:00 -7000 2015",
      "user": {
        "follower_count": 161770,
        "screen_name": "marceloclaure",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@jalopeo @sprintcare @JohnLegere @sprint hi Jalopeo I wouldn't worry he does that to anyone  that writes something he doesn't like.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "JohnLegere",
          },
          {
            "screen_name": "sprintcare",
          },
          {
            "screen_name": "sprint",
          },
          {
            "screen_name": "jalopeo",
          }
        ]
      },

      "retweet_count": 3,
      "favourites_count": 2,
      "created_at": "Sat Feb 4 19:57:00 -7000 2015",
      "user": {
        "follower_count": 161770,
        "screen_name": "marceloclaure",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@mirizarry17 @Punisher2892 @JohnLegere @TMobile LOL common tactics !!",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "mirizarry17",
          },
          {
            "screen_name": "Punisher2892",
          },
          {
            "screen_name": "JohnLegere",
          },
          {
            "screen_name": "TMobile",
          }
        ]
      },
      "retweet_count": 0,
      "favourites_count": 2,
      "created_at": "Sat Feb 4 20:00:00 -7000 2015",
      "user": {
        "follower_count": 161770,
        "screen_name": "marceloclaure",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@77978e8a1e39413 @JohnLegere @TMobileHelp @sprint hey Robert very happy. Look at the profile picture well, very happy surrounded by family",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "77978e8a1e39413",
          },
          {
            "screen_name": "TMobileHelp",
          },
          {
            "screen_name": "JohnLegere",
          },
          {
            "screen_name": "sprint",
          }
        ]
      },
      "retweet_count": 0,
      "favourites_count": 0,
      "created_at": "Sat Feb 4 20:04:00 -7000 2015",
      "user": {
        "follower_count": 161770,
        "screen_name": "marceloclaure",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@RogerWCheng @JohnLegere Hi Roger, I manage my own account, but I am more interested in answering customer issues than tweeting meaningless",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "RogerWCheng",
          },
          {
            "screen_name": "JohnLegere",
          }
        ]
      },
      "retweet_count": 9,
      "favourites_count": 22,
      "created_at": "Sat Feb 4 20:14:00 -7000 2015",
      "user": {
        "follower_count": 161770,
        "screen_name": "marceloclaure",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@JohnLegere @sprint @TMobile hmmm I will just let the facts speak for themselves 293,000 miles of network testing.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "sprint",
          },
          {
            "screen_name": "JohnLegere",
          },
          {
            "screen_name": "TMobile",
          }
        ]
      },
      "retweet_count": 47,
      "favourites_count": 62,
      "created_at": "Sat Feb 10 20:39:00 -7000 2015",
      "user": {
        "follower_count": 161770,
        "screen_name": "marceloclaure",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@JohnLegere You have to stop reacting like a Tasmanian Devil. No more tweets. I am busy improving my network..you should do the same.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "JohnLegere",
          }
        ]
      },
      "retweet_count": 73,
      "favourites_count": 118,
      "created_at": "Sat Feb 10 20:58:00 -7000 2015",
      "user": {
        "follower_count": 161770,
        "screen_name": "marceloclaure",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@surfprescott @youlookfoolish @sprint @TMobile @JohnLegere very classy",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "JohnLegere",
          },
          {
            "screen_name": "surfprescott",
          },
          {
            "screen_name": "youlookfoolish",
          },
          {
            "screen_name": "sprint",
          },
          {
            "screen_name": "TMobile",
          }
        ]
      },
      "retweet_count": 1,
      "favourites_count": 4,
      "created_at": "Sat Feb 13 19:05:00 -7000 2015",
      "user": {
        "follower_count": 161770,
        "screen_name": "marceloclaure",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@JohnLegere Hi John it's very simple and transparent $80 a month it includes a Galaxy S6, unlimited talk, text and data, you can upgrade",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "JohnLegere",
          }
        ]
      },
      "retweet_count": 36,
      "favourites_count": 51,
      "created_at": "Sat Mar 26 19:34:00 -7000 2015",
      "user": {
        "follower_count": 161770,
        "screen_name": "marceloclaure",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@JohnLegere your phone every single year. What is it so hard to understand ? Simple, clear and transparent.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "JohnLegere",
          }
        ]
      },
      "retweet_count": 17,
      "favourites_count": 31,
      "created_at": "Sat Mar 26 19:35:00 -7000 2015",
      "user": {
        "follower_count": 161770,
        "screen_name": "marceloclaure",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@EssentialDacome @JohnLegere not at Sprint. Unlimited is truly unlimited.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "JohnLegere",
          },
          {
            "screen_name": "EssentialDacome",
          }
        ]
      },
      "retweet_count": 11,
      "favourites_count": 15,
      "created_at": "Sat Mar 26 20:48:00 -7000 2015",
      "user": {
        "follower_count": 161770,
        "screen_name": "marceloclaure",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@JohnLegere no more tweets...will let the numbers speak AGAIN. I am sure lots of people love to save 450 a year ",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "JohnLegere",
          }
        ]
      },
      "retweet_count": 183,
      "favourites_count": 167,
      "created_at": "Sat Mar 26 21:14:00 -7000 2015",
      "user": {
        "follower_count": 161770,
        "screen_name": "marceloclaure",
        "time_zone": "Pacific",
      },
    },

    {
      "text": "@mlg823 @JohnLegere Mathew our new leasing programs let's you have the latest devices every year not 2 years like the rest of the carriers.",
      "entities": {
        "user_mentions": [
          {
            "screen_name": "JohnLegere",
          },
          {
            "screen_name": "mlg823",
          }
        ]
      },
      "retweet_count": 3,
      "favourites_count": 4,
      "created_at": "Sat Mar 27 17:23:00 -7000 2015",
      "user": {
        "follower_count": 161770,
        "screen_name": "marceloclaure",
        "time_zone": "Pacific",
      },
    },
  ]
  );
  module.tweets = tweets;
})(app);
